...
export default function ListaPremios (){
    ...
    const handleOkDelete = async () => {
        ...
        if (respostaJson.status !== 200){
            message.error('ERRO!! Prêmio não foi deletado!');
        }else if (respostaJson.status === 200){
            message.success('Sucesso!! Prêmio deletado!');
        }
        await getPremios();
    ...
    const handleOkEdit = async (descricao,pontos, quantidade ) => {
        ...
        const url = "http://localhost:4000/premio/" + idSelecionado;
        const param = { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(body) }
        const resposta = await fetch(url, param); 
        const respostaJson = await resposta.json();
        console.log(respostaJson)
        if (respostaJson.status !== 200){
            message.error('ERRO!! Prêmio não editado!');
        }else if (respostaJson.status === 200){
            message.success('Sucesso!! Prêmio editado!');
        }
        await getPremios();
